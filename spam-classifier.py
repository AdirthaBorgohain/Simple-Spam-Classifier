...
class SpamClassifier(object):
    ...
    def calc_TF_and_IDF(self):
        ...
        self.spam_mails, self.ham_mails = self.labels.value_counts()[
    ...
    def classify(self, processed_message):
        ...
        for word in processed_message:
            if word in self.prob_spam:
                ...
                if self.method == 'tf-idf':
                    pSpam -= log(self.sum_tf_idf_spam +
                                 len(list(self.prob_spam.keys())))
                else:
                    pSpam -= log(self.spam_words +
                                 len(list(self.prob_spam.keys())))
            if word in self.prob_ham:
                pHam += log(self.prob_ham[word])
            else:
                if self.method == 'tf-idf':
                    pHam -= log(self.sum_tf_idf_ham +
                                len(list(self.prob_ham.keys())))
                else:
                    pHam -= log(self.ham_words +
                                len(list(self.prob_ham.keys())))
